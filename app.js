...
Tu \xE9s Dom Bosco n\xF3s somos os teus queridos jovens amados.
                                                ...
                                                11`}],S={async get(o){try{const{data:e}=await SB.from("missa_kv").select("value").eq("key",o).maybeSingle();return e?e.value:null}catch(e){return console.error(e),null}},async set(o,e){try{return await SB.from("missa_kv").upsert({key:o,value:e,updated_at:new Date().toISOString()}),!0}catch(n){return console.error(n),!1}}},uid=o=>(o||"u")+Date.now().toString(36)+Math.random().toString(36).slice(2,6),norm=o=>(o||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,""),nt=o=>norm(o).replace(/[^a-z0-9 ]+/g," ").replace(/\s+/g," ").trim(),driveEmbed=o=>{const e=(o||"").match(/\/d\/([A-Za-z0-9_-]+)/);return e?`https://drive.google.com/file/d/${e[1]}/preview`:o};function ymd(o){const e=o.getFullYear(),n=String(o.getMonth()+1).padStart(2,"0"),t=String(o.getDate()).padStart(2,"0");return`${e}-${n}-${t}`}function parseYmd(o){const[e,n,t]=o.split("-").map(Number);return new Date(e,n-1,t)}function startOfWeek(o){const e=new Date(o);e.setHours(0,0,0,0);const n=(e.getDay()+6)%7;return e.setDate(e.getDate()-n),e}function fmtDiaCurto(o){return`${DOW_NOME[o.getDay()].slice(0,3)} ${String(o.getDate()).padStart(2,"0")}/${String(o.getMonth()+1).padStart(2,"0")}`}function fmtDiaLongo(o){return`${DOW_NOME[o.getDay()]}, ${String(o.getDate()).padStart(2,"0")}/${String(o.getMonth()+1).padStart(2,"0")}/${o.getFullYear()}`}const SH=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"],ENH={Db:"C#",Eb:"D#",Gb:"F#",Ab:"G#",Bb:"A#",Cb:"B",Fb:"E","E#":"F","B#":"C"};function shiftRoot(o,e){let n=ENH[o]||o,t=SH.indexOf(n);return t<0?o:SH[((t+e)%12+12)%12]}const CH_RE=/^([A-G](?:#|b)?)((?:maj|min|sus|add|dim|aug|m|M|°|º|\+|\d|\(|\)|b|#|\.)*)(?:\/([A-G](?:#|b)?))?$/;function isChordToken(o){return o?!!o.match(CH_RE):!1}function transposeToken(o,e){const n=o.match(CH_RE);if(!n)return o;let t=shiftRoot(n[1],e)+(n[2]||"");return n[3]&&(t+="/"+shiftRoot(n[3],e)),t}function isChordLine(o){const e=o.trim().split(/\s+/).filter(Boolean);return e.length?e.filter(isChordToken).length/e.length>=.6:!1}function transposeText(o,e){return!e||!o?o:o.split(`
`).map(n=>isChordLine(n)?n.replace(/\S+/g,t=>isChordToken(t)?transposeToken(t,e):t):n).join(`
`)}function App(){const[o,e]=useState(!0),[n,t]=useState("repertorio"),[i,E]=useState({}),[h,v]=useState([]),[r,f]=useState(DEFAULT_TEAMS),[l,m]=useState({}),[A,u]=useState({font:17}),[p,D]=useState(null),[c,b]=useState(null),[x,O]=useState(null),[q,M]=useState(null),[a,G]=useState(null),[F,k]=useState(null),[N,w]=useState(null),[R,I]=useState(()=>{try{return localStorage.getItem("mt_editor")==="1"}catch(s){return!1}}),[$,P]=useState(!1),U=useRef({}),V=useCallback(async()=>{const[s,d,B,Q,ro,mo,io]=await Promise.all([S.get("mt_over"),S.get("mt_added"),S.get("mt_teams"),S.get("mt_masses"),S.get("mt_settings"),S.get("mt_seedV"),S.get("mt_pin")]);mo!==SEED_VERSION&&await S.set("mt_seedV",SEED_VERSION),E(s||{}),v(d||[]),f(B&&B.length?B:DEFAULT_TEAMS),m(Q||{}),u(z({font:17},ro||{})),w(io||null),e(!1)},[]);useEffect(()=>{V();const s=SB.channel("missa_kv_rt").on("postgres_changes",{event:"*",schema:"public",table:"missa_kv"},()=>V()).subscribe();return()=>{try{SB.removeChannel(s)}catch(d){}}},[V]);const L=(s,d,B)=>{B(d),U.current[s]&&clearTimeout(U.current[s]),U.current[s]=setTimeout(()=>S.set(s,d),600)},J=s=>L("mt_over",s,E),H=s=>L("mt_added",s,v),oo=s=>L("mt_teams",s,f),eo=s=>L("mt_masses",s,m),y=s=>L("mt_settings",s,u),j=useMemo(()=>SEED_SONGS.filter(d=>!(i[d.id]&&i[d.id].hidden)).map(d=>z(z({},d),i[d.id]||{})).concat(h),[i,h]),T=useMemo(()=>{const s={};return j.forEach(d=>s[d.id]=d),s},[j]),_=s=>{const d=T[s];d&&(d.cifra?D(s):d.u?M({url:d.u,title:(d.n!=null?d.n+" \xB7 ":"")+d.t}):D(s))},ao=(s,d)=>{const B=(s||[]).filter(Q=>T[Q]);B.length&&b({ids:B,i:Math.max(0,Math.min(d||0,B.length-1))})},Z=(s,d)=>{s.startsWith("u")?H(h.map(B=>B.id===s?z(z({},B),d):B)):J(g(z({},i),{[s]:z(z({},i[s]||{}),d)}))},no=s=>{const d=z({id:uid("u"),src:"Adicionada"},s);return H([...h,d]),d.id},Ao=s=>{s.startsWith("u")?H(h.filter(d=>d.id!==s)):J(g(z({},i),{[s]:g(z({},i[s]||{}),{hidden:!0})}))},so=s=>{const d=(s||"").trim();if(!N){if(d.length<4)return"Defina uma senha com ao menos 4 d\xEDgitos.";S.set("mt_pin",d),w(d),I(!0);try{localStorage.setItem("mt_editor","1")}catch(B){}return""}if(d===N){I(!0);try{localStorage.setItem("mt_editor","1")}catch(B){}return""}return"Senha incorreta."},K=()=>{I(!1);try{localStorage.removeItem("mt_editor")}catch(s){}};return o?React.createElement("div",{style:{background:C.bg,color:C.txt},className:"min-h-screen flex items-center justify-center p-6"},React.createElement("div",{className:"text-center"},React.createElement("div",{style:{background:"#fff"},className:"rounded-2xl p-3 mb-4 inline-block shadow-lg"},React.createElement("img",{src:"church.png",alt:"Par\xF3quia Santa Teresinha",style:{maxWidth:300,width:"72vw"}})),React.createElement("p",{style:{color:C.mut},className:"flex items-center justify-center gap-2"},React.createElement(Music,{size:18,style:{color:C.gold},className:"animate-pulse"})," Carregando o repert\xF3rio\u2026"))):React.createElement("div",{style:{background:C.bg,color:C.txt,minHeight:"100vh",fontFamily:"system-ui, sans-serif"}},React.createElement("div",{style:{background:`linear-gradient(135deg, ${C.card2}, ${C.card})`,borderBottom:`1px solid ${C.line}`},className:"px-4 pt-4 pb-3 sticky top-0 z-20"},React.createElement("div",{className:"flex items-center gap-2"},React.createElement("div",{style:{background:"#fff"},className:"rounded-lg px-1.5 py-1 shrink-0"},React.createElement("img",{src:"church.png",alt:"Par\xF3quia Santa Teresinha",style:{height:30}})),React.createElement("div",{className:"leading-tight"},React.createElement("h1",{className:"font-bold"},"Repert\xF3rio da Missa"),React.createElement("p",{className:"text-xs",style:{color:C.mut}},"Santa Teresinha \xB7 ",j.length," m\xFAsicas")),React.createElement("button",{onClick:()=>R?K():P(!0),title:R?"Sair do modo edi\xE7\xE3o":"Entrar no modo edi\xE7\xE3o",style:{background:C.card2,color:R?C.gold:C.mut,marginLeft:"auto"},className:"px-2.5 py-2 rounded-lg flex items-center gap-1 text-xs shrink-0"},R?React.createElement(Unlock,{size:15}):React.createElement(Lock,{size:15}),R?React.createElement("span",null,"edi\xE7\xE3o"):null))),React.createElement("div",{className:"p-3 pb-24 max-w-2xl mx-auto"},n==="repertorio"&&React.createElement(Repertorio,{songs:j,onPlay:_,onAdd:no,onEdit:s=>k(s),onVideo:s=>G({url:s.yt,title:(s.n!=null?s.n+" \xB7 ":"")+s.t}),editor:R}),n==="missas"&&React.createElement(Missas,{masses:l,teams:r,songById:T,onOpen:s=>O(s)}),n==="equipes"&&React.createElement(Equipes,{teams:r,saveTeams:oo,masses:l,editor:R,onOpenLock:()=>P(!0)}),n==="ajustes"&&React.createElement(Ajustes,{settings:A,saveSettings:y,songs:j,editor:R,hasPin:!!N,pin:N,onLock:K,onOpenLock:()=>P(!0),savePin:s=>{S.set("mt_pin",s),w(s)}})),React.createElement("nav",{style:{background:C.card,borderTop:`1px solid ${C.line}`},className:"fixed bottom-0 left-0 right-0 z-20"},React.createElement("div",{className:"max-w-2xl mx-auto grid grid-cols-4"},[{k:"repertorio",ic:Music,lb:"Repert\xF3rio"},{k:"missas",ic:Calendar,lb:"Missas"},{k:"equipes",ic:Users,lb:"Equipes"},{k:"ajustes",ic:Settings,lb:"Ajustes"}].map(({k:s,ic:d,lb:B})=>React.createElement("button",{key:s,onClick:()=>t(s),className:"py-2.5 flex flex-col items-center gap-0.5",style:{color:n===s?C.gold:C.mut}},React.createElement(d,{size:20}),React.createElement("span",{style:{fontSize:10.5}},B))))),(p||c)&&React.createElement(Player,{song:c?T[c.ids[c.i]]:T[p],playlist:c?c.ids:null,index:c?c.i:0,onNav:s=>b(d=>d&&g(z({},d),{i:Math.max(0,Math.min(s,d.ids.length-1))})),settings:A,saveSettings:y,updateSong:Z,openViewer:(s,d)=>M({url:s,title:d}),editor:R,onClose:()=>{c?b(null):D(null)}}),q&&React.createElement(PdfViewer,{url:q.url,title:q.title,onClose:()=>M(null)}),a&&React.createElement(VideoViewer,{url:a.url,title:a.title,onClose:()=>G(null)}),F&&React.createElement(AddSong,{initial:F,onClose:()=>k(null),onSave:s=>{Z(F.id,s),k(null)}}),$&&React.createElement(LockModal,{hasPin:!!N,onClose:()=>P(!1),onSubmit:s=>so(s)}),x&&React.createElement(MassEditor,{mkey:x,masses:l,saveMasses:eo,teams:r,songs:j,songById:T,onPlay:_,onShow:ao,onVideo:s=>G({url:s.yt,title:s.t}),onEdit:s=>k(s),editor:R,onClose:()=>O(null)}))}function Repertorio({songs:o,onPlay:e,onAdd:n,onEdit:t,onVideo:i,editor:E}){const[h,v]=useState(""),[r,f]=useState("Todos"),[l,m]=useState("Todas"),[A,u]=useState(!1),p=useMemo(()=>["Todas",...Array.from(new Set(o.map(c=>c.src)))],[o]),D=useMemo(()=>{const c=nt(h),b=c.split(" ").filter(Boolean),x=h.replace(/\D/g,"");let O=o.filter(q=>{if(r!=="Todos"&&q.m!==r||l!=="Todas"&&q.src!==l)return!1;if(!b.length&&!x)return!0;const M=nt(q.t),a=b.length>0&&b.every(F=>M.includes(F)),G=x!==""&&q.n!=null&&String(q.n).includes(x);return a||G});if(b.length||x){const q=M=>{const a=nt(M.t);return x&&String(M.n)===x?0:a===c?1:c&&a.startsWith(c)?2:b[0]&&a.startsWith(b[0])?3:c&&a.includes(c)?4:5};O=O.slice().sort((M,a)=>q(M)-q(a)||M.t.localeCompare(a.t,"pt"))}return O.slice(0,200)},[o,h,r,l]);return React.createElement("div",null,React.createElement("div",{className:"relative mb-2"},React.createElement(Search,{size:16,style:{color:C.mut},className:"absolute left-3 top-1/2 -translate-y-1/2"}),React.createElement("input",{value:h,onChange:c=>v(c.target.value),placeholder:"Buscar por t\xEDtulo ou n\xBA\u2026",style:{background:C.card,border:`1px solid ${C.line}`,color:C.txt},className:"w-full pl-9 pr-3 py-2.5 rounded-xl outline-none"})),React.createElement("div",{className:"flex gap-2 overflow-x-auto pb-2"},React.createElement(Sel,{value:r,onChange:f,options:["Todos",...MOMENTOS]}),React.createElement(Sel,{value:l,onChange:m,options:p}),E&&React.createElement("button",{onClick:()=>u(!0),style:{background:C.gold,color:"#2b2100"},className:"px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap flex items-center gap-1"},React.createElement(Plus,{size:14})," Nova")),React.createElement("p",{className:"text-xs mb-2",style:{color:C.mut}},D.length," resultado(s)",D.length===400?"+ (refine a busca)":""),React.createElement("div",{className:"space-y-2"},D.map(c=>React.createElement(SongRow,{key:c.id,s:c,onPlay:e,onVideo:i,onEdit:t,editor:E}))),A&&React.createElement(AddSong,{onClose:()=>u(!1),onSave:c=>{n(c),u(!1)}}))}function SongRow({s:o,onPlay:e,onVideo:n,onEdit:t,editor:i}){return React.createElement("div",{style:{background:C.card,border:`1px solid ${C.line}`},className:"w-full p-3 rounded-xl flex items-center gap-2"},React.createElement("button",{onClick:()=>e(o.id),className:"flex items-center gap-3 min-w-0 flex-1 text-left active:opacity-70"},React.createElement("div",{style:{background:C.card2,color:C.gold,minWidth:44},className:"px-2 py-1 rounded-lg text-center"},o.n!=null?React.createElement("span",{className:"font-bold text-sm"},o.n):React.createElement(Music,{size:16,className:"mx-auto"})),React.createElement("div",{className:"min-w-0 flex-1"},React.createElement("p",{className:"font-semibold truncate"},o.t),React.createElement("div",{className:"flex items-center gap-2 text-xs mt-0.5",style:{color:C.mut}},o.tom?React.createElement("span",{style:{color:C.violet}},"\u{1F3B5} ",o.tom):null,React.createElement("span",{style:{color:MOMENTO_COR[o.m]||C.mut}},o.m)))),o.yt?React.createElement("button",{onClick:()=>n(o),title:"Assistir v\xEDdeo",style:{background:C.card2,color:C.danger},className:"p-2 rounded-lg shrink-0"},React.createElement(Youtube,{size:18})):null,o.sp?React.createElement("a",{href:o.sp,target:"_blank",rel:"noreferrer",onClick:E=>E.stopPropagation(),title:"Ouvir no Spotify",style:{background:C.card2,color:C.green},className:"p-2 rounded-lg shrink-0"},React.createElement(Spotify,{size:18})):null,i?React.createElement("button",{onClick:()=>t(o),title:"Editar cadastro",style:{background:C.card2,color:C.gold},className:"p-2 rounded-lg shrink-0"},React.createElement(Pencil,{size:16})):null,React.createElement("button",{onClick:()=>e(o.id),title:"Abrir cifra",style:{color:C.gold},className:"p-1.5 shrink-0"},o.cifra?React.createElement(Play,{size:18}):React.createElement(FileText,{size:17,style:{color:C.blue}})))}function Sel({value:o,onChange:e,options:n}){return React.createElement("select",{value:o,onChange:t=>e(t.target.value),style:{background:C.card,border:`1px solid ${C.line}`,color:C.txt},className:"px-3 py-1.5 rounded-full text-xs outline-none max-w-[46%]"},n.map(t=>React.createElement("option",{key:t,value:t},t)))}function Player({song:o,settings:e,saveSettings:n,updateSong:t,openViewer:i,editor:E,onClose:h,playlist:v,index:r,onNav:f}){const[l,m]=useState(0),[A,u]=useState(0),[p,D]=useState(!1),[c,b]=useState(o?{tom:o.tom||"",m:o.m||"Geral",u:o.u||"",sp:o.sp||"",cifra:o.cifra||""}:{}),x=useRef(null),O=e.font||17;if(useEffect(()=>{m(0),u(0),D(!1),b({tom:(o==null?void 0:o.tom)||"",m:(o==null?void 0:o.m)||"Geral",u:(o==null?void 0:o.u)||"",sp:(o==null?void 0:o.sp)||"",cifra:(o==null?void 0:o.cifra)||""})},[o==null?void 0:o.id]),useEffect(()=>{if(!A||!x.current)return;const a=x.current,G=setInterval(()=>{a.scrollTop+=A},60);return()=>clearInterval(G)},[A]),useEffect(()=>{let a=null;const G=async()=>{try{navigator.wakeLock&&(a=await navigator.wakeLock.request("screen"))}catch(k){}};G();const F=()=>{document.visibilityState==="visible"&&G()};return document.addEventListener("visibilitychange",F),()=>{document.removeEventListener("visibilitychange",F);try{a&&a.release()}catch(k){}}},[]),!o)return null;const M=(o.cifra?transposeText(o.cifra,l):"").split(`
`);return React.createElement("div",{style:{background:C.bg,color:C.txt},className:"fixed inset-0 z-[60] flex flex-col"},React.createElement("div",{style:{background:C.card,borderBottom:`1px solid ${C.line}`},className:"px-3 py-2 flex items-center gap-2 sticky top-0"},React.createElement("button",{onClick:h,style:{background:C.card2,color:C.txt},className:"px-3 py-2 rounded-lg flex items-center gap-1 font-semibold shrink-0"},React.createElement(ChevronLeft,{size:18})," Voltar"),React.createElement("div",{className:"min-w-0 flex-1"},React.createElement("p",{className:"font-bold truncate"},o.n!=null?o.n+" \xB7 ":"",o.t),React.createElement("p",{className:"text-xs truncate",style:{color:C.mut}},o.src,o.tom?" \xB7 tom original "+o.tom:"")),v&&v.length>1?React.createElement("div",{className:"flex items-center gap-1 shrink-0"},React.createElement("button",{onClick:()=>f(r-1),disabled:r<=0,style:{background:C.card2,opacity:r<=0?.4:1},className:"p-2 rounded-lg"},React.createElement(ChevronLeft,{size:16})),React.createElement("span",{className:"text-xs font-semibold text-center",style:{color:C.mut,minWidth:36}},r+1,"/",v.length),React.createElement("button",{onClick:()=>f(r+1),disabled:r>=v.length-1,style:{background:C.card2,opacity:r>=v.length-1?.4:1},className:"p-2 rounded-lg"},React.createElement(ChevronRight,{size:16}))):null,E?React.createElement("button",{onClick:()=>D(a=>!a),style:{background:C.card2,color:p?C.gold:C.txt},className:"p-2 rounded-lg"},React.createElement(Pencil,{size:16})):null),React.createElement("div",{style:{background:C.card2,borderBottom:`1px solid ${C.line}`},className:"px-3 py-2 flex items-center gap-2 flex-wrap text-xs"},React.createElement("div",{className:"flex items-center gap-1"},React.createElement("span",{style:{color:C.mut}},"Tom"),React.createElement("button",{onClick:()=>m(a=>a-1),style:{background:C.card},className:"p-1.5 rounded-lg"},React.createElement(Minus,{size:14})),React.createElement("span",{style:{color:C.gold,minWidth:26},className:"text-center font-bold"},l>0?"+"+l:l),React.createElement("button",{onClick:()=>m(a=>a+1),style:{background:C.card},className:"p-1.5 rounded-lg"},React.createElement(Plus,{size:14}))),React.createElement("div",{className:"flex items-center gap-1"},React.createElement(Type,{size:13,style:{color:C.mut}}),React.createElement("button",{onClick:()=>n(g(z({},e),{font:Math.max(12,O-1)})),style:{background:C.card},className:"p-1.5 rounded-lg"},React.createElement(Minus,{size:14})),React.createElement("span",{style:{minWidth:22},className:"text-center"},O),React.createElement("button",{onClick:()=>n(g(z({},e),{font:Math.min(30,O+1)})),style:{background:C.card},className:"p-1.5 rounded-lg"},React.createElement(Plus,{size:14}))),React.createElement("div",{className:"flex items-center gap-1"},React.createElement("button",{onClick:()=>u(a=>a>0?0:1),style:{background:A?C.gold:C.card,color:A?"#2b2100":C.txt},className:"p-1.5 rounded-lg flex items-center gap-1"},A?React.createElement(Pause,{size:14}):React.createElement(Play,{size:14}),React.createElement("span",null,"rolar")),A>0&&React.createElement("input",{type:"range",min:"1",max:"5",value:A,onChange:a=>u(Number(a.target.value)),className:"w-16"})),React.createElement("div",{className:"ml-auto flex items-center gap-2"},o.sp?React.createElement("a",{href:o.sp,target:"_blank",rel:"noreferrer",title:"Ouvir no Spotify",style:{background:C.green,color:"#04261b"},className:"px-2.5 py-1.5 rounded-lg font-semibold flex items-center gap-1"},React.createElement(Spotify,{size:14})," Spotify"):null,o.u?React.createElement("button",{onClick:()=>i(o.u,o.t),style:{background:C.blue,color:"#04203f"},className:"px-2.5 py-1.5 rounded-lg font-semibold flex items-center gap-1"},React.createElement(FileText,{size:13})," Ver cifra"):null)),p?React.createElement("div",{className:"p-3 overflow-y-auto flex-1 space-y-3"},React.createElement("p",{className:"text-sm font-semibold",style:{color:C.gold}},"Editar m\xFAsica"),React.createElement(Field,{label:"Tom"},React.createElement("input",{value:c.tom,onChange:a=>b(g(z({},c),{tom:a.target.value})),placeholder:"ex.: Sol Maior / G",style:inp})),React.createElement(Field,{label:"Momento da missa"},React.createElement("select",{value:c.m,onChange:a=>b(g(z({},c),{m:a.target.value})),style:inp},MOMENTOS.map(a=>React.createElement("option",{key:a},a)))),React.createElement(Field,{label:"Link (Drive/YouTube) \u2014 opcional"},React.createElement("input",{value:c.u,onChange:a=>b(g(z({},c),{u:a.target.value})),placeholder:"https://\u2026",style:inp})),React.createElement(Field,{label:"Link do Spotify \u2014 opcional"},React.createElement("input",{value:c.sp||"",onChange:a=>b(g(z({},c),{sp:a.target.value})),placeholder:"https://open.spotify.com/track/\u2026",style:inp})),React.createElement(Field,{label:"Cifra em texto (acordes + letra) \u2014 habilita transpor/rolar"},React.createElement("textarea",{value:c.cifra,onChange:a=>b(g(z({},c),{cifra:a.target.value})),rows:10,wrap:"off",placeholder:`Cole aqui a cifra. Ex.:
   C        G
Vem, Esp\xEDrito Santo\u2026`,style:g(z({},inp),{fontFamily:"ui-monospace, monospace",whiteSpace:"pre",overflowX:"auto"})})),React.createElement("div",{className:"flex gap-2"},React.createElement("button",{onClick:()=>{t(o.id,c),D(!1)},style:{background:C.green,color:"#04261b"},className:"flex-1 py-2.5 rounded-xl font-semibold flex items-center justify-center gap-1"},React.createElement(Save,{size:16})," Salvar"),React.createElement("button",{onClick:()=>D(!1),style:{background:C.card2,color:C.mut},className:"px-4 rounded-xl"},"Cancelar"))):React.createElement("div",{ref:x,className:"flex-1 px-3 py-3",style:{overflow:"auto",scrollBehavior:"smooth",WebkitOverflowScrolling:"touch"}},o.cifra?React.createElement("pre",{style:{fontFamily:"ui-monospace, SFMono-Regular, Menlo, monospace",fontSize:O,lineHeight:1.5,whiteSpace:"pre",display:"inline-block",minWidth:"100%"}},M.map((a,G)=>isChordLine(a)?React.createElement("div",{key:G,style:{color:C.gold,fontWeight:700}},a||"\xA0"):React.createElement("div",{key:G},a||"\xA0"))):o.u?React.createElement("div",{className:"text-center py-6"},React.createElement(FileText,{size:42,style:{color:C.blue},className:"mx-auto mb-3"}),React.createElement("p",{className:"mb-3",style:{color:C.mut}},"A cifra desta m\xFAsica est\xE1 em PDF no Drive."),React.createElement("button",{onClick:()=>i(o.u,o.t),style:{background:C.blue,color:"#04203f"},className:"inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold"},React.createElement(FileText,{size:16})," Ver cifra (PDF)"),React.createElement("p",{className:"mt-4 text-xs",style:{color:C.mut}},"Dica: toque no l\xE1pis \u270F\uFE0F acima para colar a cifra em texto e habilitar transpor/rolagem.")):React.createElement("div",{className:"text-center py-6"},React.createElement(ListMusic,{size:42,style:{color:C.mut},className:"mx-auto mb-3"}),React.createElement("p",{className:"mb-3",style:{color:C.mut}},"Ainda sem cifra cadastrada."),E?React.createElement("button",{onClick:()=>D(!0),style:{background:C.gold,color:"#2b2100"},className:"inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold"},React.createElement(Pencil,{size:16})," Colar cifra ou link"):React.createElement("p",{className:"text-xs",style:{color:C.mut}},"Entre no modo edi\xE7\xE3o (cadeado no topo) para adicionar a cifra."))))}function Missas({masses:o,teams:e,songById:n,onOpen:t}){const[i,E]=useState(()=>startOfWeek(new Date)),h=useMemo(()=>Array.from({length:7},(l,m)=>{const A=new Date(i);return A.setDate(i.getDate()+m),A}),[i]),v=useMemo(()=>{const l={};return e.forEach(m=>l[m.id]=m),l},[e]),r=l=>{const m=new Date(i);m.setDate(i.getDate()+l*7),E(m)},f=ymd(new Date);return React.createElement("div",null,React.createElement("div",{className:"flex items-center justify-between mb-3"},React.createElement("button",{onClick:()=>r(-1),style:{background:C.card},className:"p-2 rounded-lg"},React.createElement(ChevronLeft,{size:18})),React.createElement("div",{className:"text-center"},React.createElement("p",{className:"text-xs",style:{color:C.mut}},"Semana de"),React.createElement("p",{className:"font-semibold text-sm"},fmtDiaCurto(h[0])," \u2014 ",fmtDiaCurto(h[6]))),React.createElement("button",{onClick:()=>r(1),style:{background:C.card},className:"p-2 rounded-lg"},React.createElement(ChevronRight,{size:18}))),React.createElement("button",{onClick:()=>E(startOfWeek(new Date)),style:{color:C.gold},className:"text-xs mb-3 underline"},"Voltar para esta semana"),h.map(l=>{const m=SLOTS_DOW[l.getDay()]||[],A=ymd(l)===f;return React.createElement("div",{key:ymd(l),className:"mb-4"},React.createElement("h2",{className:"font-bold text-sm mb-2 flex items-center gap-2",style:{color:A?C.gold:C.txt}},fmtDiaLongo(l)," ",A&&React.createElement("span",{style:{background:C.gold,color:"#2b2100"},className:"px-2 py-0.5 rounded-full text-[10px]"},"hoje")),React.createElement("div",{className:"space-y-2"},m.map(u=>{const p=ymd(l)+"|"+u,D=o[p],c=D&&D.team?v[D.team]:null,b=D&&D.songs?D.songs.length:0;return React.createElement("button",{key:u,onClick:()=>t(p),style:{background:C.card,border:`1px solid ${c?c.color:C.line}`},className:"w-full text-left p-3 rounded-xl flex items-center gap-3 active:opacity-70"},React.createElement("div",{style:{background:C.card2,color:C.gold,minWidth:56},className:"px-2 py-1.5 rounded-lg text-center font-bold text-sm"},u),React.createElement("div",{className:"min-w-0 flex-1"},c?React.createElement("p",{className:"font-semibold truncate",style:{color:c.color}},c.name):React.createElement("p",{style:{color:C.mut}},"Sem equipe definida"),React.createElement("p",{className:"text-xs",style:{color:C.mut}},b>0?b+" m\xFAsica(s) no repert\xF3rio":"toque para montar a missa")),React.createElement(ChevronRight,{size:18,style:{color:C.mut}}))})))}))}function MassEditor({mkey:o,masses:e,saveMasses:n,teams:t,songs:i,songById:E,onPlay:h,onShow:v,onVideo:r,onEdit:f,editor:l,onClose:m}){const[A,u]=o.split("|"),p=e[o]||{team:"",songs:[],note:""},[D,c]=useState(!1),b=a=>n(g(z({},e),{[o]:z(z({},p),a)})),x=(p.songs||[]).map(a=>E[a]).filter(Boolean),O=x.map(a=>a.id),q=(a,G)=>{const F=[...p.songs],k=a+G;k<0||k>=F.length||([F[a],F[k]]=[F[k],F[a]],b({songs:F}))},M=a=>b({songs:p.songs.filter((G,F)=>F!==a)});return React.createElement("div",{style:{background:C.bg,color:C.txt},className:"fixed inset-0 z-40 flex flex-col"},React.createElement("div",{style:{background:C.card,borderBottom:`1px solid ${C.line}`},className:"px-3 py-2 flex items-center gap-2 sticky top-0"},React.createElement("button",{onClick:m,style:{background:C.card2,color:C.txt},className:"px-3 py-2 rounded-lg flex items-center gap-1 font-semibold shrink-0"},React.createElement(ChevronLeft,{size:18})," Voltar"),React.createElement("div",null,React.createElement("p",{className:"font-bold"},fmtDiaLongo(parseYmd(A))),React.createElement("p",{className:"text-xs",style:{color:C.mut}},"Missa das ",u))),React.createElement("div",{className:"overflow-y-auto flex-1 p-3 space-y-4"},React.createElement(Field,{label:"Equipe que vai tocar"},React.createElement("select",{value:p.team||"",onChange:a=>b({team:a.target.value}),style:inp},React.createElement("option",{value:""},"\u2014 escolher equipe \u2014"),t.map(a=>React.createElement("option",{key:a.id,value:a.id},a.name)))),React.createElement("div",null,React.createElement("div",{className:"flex items-center justify-between mb-2"},React.createElement("p",{className:"font-semibold text-sm",style:{color:C.gold}},"Repert\xF3rio (",x.length,")"),React.createElement("div",{className:"flex items-center gap-2"},x.length>0?React.createElement("button",{onClick:()=>v(O,0),style:{background:C.green,color:"#04261b"},className:"px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1"},React.createElement(Play,{size:14})," Modo Show"):null,React.createElement("button",{onClick:()=>c(!0),style:{background:C.gold,color:"#2b2100"},className:"px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1"},React.createElement(Plus,{size:14})," Adicionar m\xFAsica"))),x.length===0&&React.createElement("p",{className:"text-xs",style:{color:C.mut}},'Nenhuma m\xFAsica ainda. Toque em "Adicionar m\xFAsica".'),React.createElement("div",{className:"space-y-2"},x.map((a,G)=>React.createElement("div",{key:a.id+G,style:{background:C.card,border:`1px solid ${C.line}`},className:"p-2.5 rounded-xl flex items-center gap-2"},React.createElement("div",{className:"flex flex-col"},React.createElement("button",{onClick:()=>q(G,-1),style:{color:C.mut},className:"p-0.5"},React.createElement(ArrowUp,{size:14})),React.createElement("button",{onClick:()=>q(G,1),style:{color:C.mut},className:"p-0.5"},React.createElement(ArrowDown,{size:14}))),React.createElement("button",{onClick:()=>v(O,G),className:"min-w-0 flex-1 text-left"},React.createElement("p",{className:"font-medium truncate text-sm"},a.t),React.createElement("p",{className:"text-xs",style:{color:C.mut}},a.tom?a.tom+" \xB7 ":"",React.createElement("span",{style:{color:MOMENTO_COR[a.m]}},a.m))),a.yt?React.createElement("button",{onClick:()=>r(a),title:"V\xEDdeo",style:{color:C.danger},className:"p-1.5"},React.createElement(Youtube,{size:16})):null,a.sp?React.createElement("a",{href:a.sp,target:"_blank",rel:"noreferrer",title:"Spotify",style:{color:C.green},className:"p-1.5"},React.createElement(Spotify,{size:15})):null,l?React.createElement("button",{onClick:()=>f(a),title:"Editar",style:{color:C.gold},className:"p-1.5"},React.createElement(Pencil,{size:15})):null,React.createElement("button",{onClick:()=>M(G),title:"Remover",style:{color:C.danger},className:"p-1.5"},React.createElement(Trash2,{size:15})))))),React.createElement(Field,{label:"Observa\xE7\xF5es (leitura, tom especial, solista\u2026)"},React.createElement("textarea",{value:p.note||"",onChange:a=>b({note:a.target.value}),rows:3,style:inp,placeholder:"Anota\xE7\xF5es da missa\u2026"})),React.createElement("button",{onClick:m,style:{background:C.green,color:"#04261b"},className:"w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 active:opacity-80"},React.createElement(Check,{size:18})," Concluir \u2014 repert\xF3rio salvo"),React.createElement("p",{className:"text-xs text-center",style:{color:C.mut}},"Tudo \xE9 salvo automaticamente e fica vis\xEDvel para a equipe. Toque numa m\xFAsica para abrir a cifra.")),D&&React.createElement(SongPicker,{songs:i,onClose:()=>c(!1),onPick:a=>{b({songs:[...p.songs||[],a]})}}))}function SongPicker({songs:o,onClose:e,onPick:n}){const[t,i]=useState(""),[E,h]=useState("Todos"),v=useMemo(()=>{const r=nt(t),f=r.split(" ").filter(Boolean),l=t.replace(/\D/g,"");let m=o.filter(A=>{if(!(E==="Todos"||A.m===E))return!1;if(!f.length&&!l)return!0;const u=nt(A.t);return f.length>0&&f.every(p=>u.includes(p))||l!==""&&A.n!=null&&String(A.n).includes(l)});if(f.length||l){const A=u=>{const p=nt(u.t);return l&&String(u.n)===l?0:p===r?1:r&&p.startsWith(r)?2:f[0]&&p.startsWith(f[0])?3:4};m=m.slice().sort((u,p)=>A(u)-A(p)||u.t.localeCompare(p.t,"pt"))}return m.slice(0,200)},[o,t,E]);return React.createElement("div",{style:{background:"rgba(0,0,0,0.6)"},className:"fixed inset-0 z-50 flex items-end sm:items-center justify-center"},React.createElement("div",{style:{background:C.card,border:`1px solid ${C.line}`},className:"w-full max-w-lg rounded-t-2xl sm:rounded-2xl max-h-[85vh] flex flex-col"},React.createElement("div",{className:"p-3 border-b",style:{borderColor:C.line}},React.createElement("div",{className:"flex items-center justify-between mb-2"},React.createElement("p",{className:"font-semibold"},"Adicionar m\xFAsica"),React.createElement("button",{onClick:e},React.createElement(X,{size:18,style:{color:C.mut}}))),React.createElement("input",{value:t,onChange:r=>i(r.target.value),placeholder:"Buscar\u2026",style:inp,autoFocus:!0}),React.createElement("div",{className:"mt-2"},React.createElement(Sel,{value:E,onChange:h,options:["Todos",...MOMENTOS]}))),React.createElement("div",{className:"overflow-y-auto p-2 space-y-1"},v.map(r=>React.createElement("button",{key:r.id,onClick:()=>{n(r.id),e()},style:{background:C.card2},className:"w-full text-left p-2.5 rounded-lg flex items-center gap-2 active:opacity-70"},React.createElement("span",{style:{color:C.gold,minWidth:34},className:"text-xs font-bold text-center"},r.n!=null?r.n:"\u266A"),React.createElement("span",{className:"min-w-0 flex-1 truncate text-sm"},r.t),React.createElement("span",{className:"text-xs",style:{color:MOMENTO_COR[r.m]}},r.m))))))}function Equipes({teams:o,saveTeams:e,masses:n,editor:t,onOpenLock:i}){const[E,h]=useState(""),v=useMemo(()=>{const u={};return Object.values(n).forEach(p=>{p.team&&(u[p.team]=(u[p.team]||0)+1)}),u},[n]),r=[C.rose,C.violet,C.green,C.gold,C.blue,C.danger],f=()=>{const u=E.trim();u&&(e([...o,{id:uid("t"),name:u,color:r[o.length%r.length]}]),h(""))},l=(u,p)=>e(o.map(D=>D.id===u?g(z({},D),{name:p}):D)),m=u=>e(o.map(p=>p.id===u?g(z({},p),{color:r[(r.indexOf(p.color)+1)%r.length]}):p)),A=u=>e(o.filter(p=>p.id!==u));return t?React.createElement("div",null,React.createElement("p",{className:"text-sm mb-3",style:{color:C.mut}},"Cadastre as equipes/minist\xE9rios de m\xFAsica. Depois \xE9 s\xF3 escalar cada equipe nas missas da semana."),React.createElement("div",{className:"flex gap-2 mb-4"},React.createElement("input",{value:E,onChange:u=>h(u.target.value),onKeyDown:u=>u.key==="Enter"&&f(),placeholder:"Nome da equipe",style:inp}),React.createElement("button",{onClick:f,style:{background:C.gold,color:"#2b2100"},className:"px-4 rounded-xl font-semibold"},"Add")),React.createElement("div",{className:"space-y-2"},o.map(u=>React.createElement("div",{key:u.id,style:{background:C.card,border:`1px solid ${C.line}`},className:"p-3 rounded-xl flex items-center gap-3"},React.createElement("button",{onClick:()=>m(u.id),style:{background:u.color,minWidth:18,height:18},className:"rounded-full shrink-0",title:"trocar cor"}),React.createElement("input",{value:u.name,onChange:p=>l(u.id,p.target.value),style:{background:"transparent",color:C.txt},className:"flex-1 outline-none font-medium"}),React.createElement("span",{className:"text-xs",style:{color:C.mut}},v[u.id]||0," missa(s)"),React.createElement("button",{onClick:()=>A(u.id),style:{color:C.danger}},React.createElement(Trash2,{size:16})))))):React.createElement("div",null,React.createElement("div",{style:{background:C.card,border:`1px solid ${C.line}`},className:"rounded-xl p-3 mb-3 flex items-center gap-2"},React.createElement(Lock,{size:16,style:{color:C.gold}}),React.createElement("p",{className:"text-xs flex-1",style:{color:C.mut}},"Para criar ou editar equipes, entre no modo edi\xE7\xE3o."),React.createElement("button",{onClick:i,style:{background:C.gold,color:"#2b2100"},className:"px-3 py-1.5 rounded-full text-xs font-semibold shrink-0"},"Entrar")),React.createElement("div",{className:"space-y-2"},o.map(u=>React.createElement("div",{key:u.id,style:{background:C.card,border:`1px solid ${C.line}`},className:"p-3 rounded-xl flex items-center gap-3"},React.createElement("span",{style:{background:u.color,minWidth:18,height:18},className:"rounded-full inline-block shrink-0"}),React.createElement("span",{className:"flex-1 font-medium"},u.name),React.createElement("span",{className:"text-xs",style:{color:C.mut}},v[u.id]||0," missa(s)")))))}function Ajustes({settings:o,saveSettings:e,songs:n,editor:t,hasPin:i,pin:E,onLock:h,onOpenLock:v,savePin:r}){const f=n.filter(m=>m.u).length,l=n.filter(m=>m.cifra).length;return React.createElement("div",{className:"space-y-4"},React.createElement("div",{style:{background:C.card,border:`1px solid ${C.line}`},className:"rounded-xl p-4 space-y-2"},React.createElement("p",{className:"font-semibold",style:{color:C.gold}},"Senha de edi\xE7\xE3o"),t?React.createElement(React.Fragment,null,React.createElement("p",{className:"text-sm",style:{color:C.green}},"\u2713 Modo edi\xE7\xE3o ativo neste aparelho."),React.createElement(PinChange,{hasPin:i,pin:E,savePin:r}),React.createElement("button",{onClick:h,style:{background:C.card2,color:C.txt},className:"px-3 py-2 rounded-lg text-sm flex items-center gap-1"},React.createElement(Lock,{size:14})," Sair do modo edi\xE7\xE3o")):React.createElement(React.Fragment,null,React.createElement("p",{className:"text-sm",style:{color:C.mut}},i?"Apenas quem tem a senha pode editar m\xFAsicas e equipes.":"Defina uma senha para liberar a edi\xE7\xE3o de m\xFAsicas e equipes."),React.createElement("button",{onClick:v,style:{background:C.gold,color:"#2b2100"},className:"px-3 py-2 rounded-lg text-sm font-semibold flex items-center gap-1"},React.createElement(Unlock,{size:14})," ",i?"Entrar no modo edi\xE7\xE3o":"Criar senha de edi\xE7\xE3o"))),React.createElement("div",{style:{background:C.card,border:`1px solid ${C.line}`},className:"rounded-xl p-4"},React.createElement("p",{className:"font-semibold mb-1",style:{color:C.gold}},"Sobre o repert\xF3rio"),React.createElement("p",{className:"text-sm",style:{color:C.mut}},n.length," m\xFAsicas no total \xB7 ",f," com PDF \xB7 ",l," com cifra em texto (transp\xF5e)."),React.createElement("p",{className:"text-xs mt-2",style:{color:C.mut}},"Fontes: ",React.createElement("b",null,"Louvemos o Senhor")," (livro cifrado, n\xBA 1\u20131172), ",React.createElement("b",null,"Cifras PDF"),", ",React.createElement("b",null,"Santa Teresinha")," e ",React.createElement("b",null,"Dom Bosco")," (Google Drive da par\xF3quia).")),React.createElement(Field,{label:`Tamanho padr\xE3o da letra: ${o.font||17}px`},React.createElement("input",{type:"range",min:"12",max:"30",value:o.font||17,onChange:m=>e(g(z({},o),{font:Number(m.target.value)})),className:"w-full"})),React.createElement("div",{style:{background:C.card,border:`1px solid ${C.line}`},className:"rounded-xl p-4 text-sm"},React.createElement("p",{className:"font-semibold mb-1",style:{color:C.gold}},"Pasta de cifras no Drive"),React.createElement("a",{href:"https://drive.google.com/drive/folders/12s0xc-gpn0EP4SIAizsmxFc4LWNOhfxc",target:"_blank",rel:"noreferrer",style:{color:C.blue},className:"inline-flex items-center gap-1 underline"},React.createElement(ExternalLink,{size:14}),' Abrir "M\xFAsicas da igreja"')),React.createElement("p",{className:"text-xs text-center",style:{color:C.mut}},"Tudo o que voc\xEA edita aqui \xE9 compartilhado com a equipe em tempo real."))}function PinChange({hasPin:o,pin:e,savePin:n}){const[t,i]=useState(!1),[E,h]=useState(""),[v,r]=useState(""),[f,l]=useState("");if(!t)return React.createElement("button",{onClick:()=>i(!0),style:{color:C.blue},className:"text-sm underline block"},"Alterar senha");const m=()=>{if(o&&E.trim()!==e){l("Senha atual incorreta.");return}if(v.trim().length<4){l("A nova senha precisa de ao menos 4 d\xEDgitos.");return}n(v.trim()),l("Senha atualizada \u2713"),h(""),r(""),setTimeout(()=>i(!1),1e3)};return React.createElement("div",{className:"space-y-2"},o?React.createElement("input",{value:E,onChange:A=>h(A.target.value),type:"password",inputMode:"numeric",placeholder:"Senha atual",style:inp}):null,React.createElement("input",{value:v,onChange:A=>r(A.target.value),type:"password",inputMode:"numeric",placeholder:"Nova senha (m\xEDn. 4 d\xEDgitos)",style:inp}),f?React.createElement("p",{className:"text-xs",style:{color:f.indexOf("\u2713")>=0?C.green:C.danger}},f):null,React.createElement("div",{className:"flex gap-2"},React.createElement("button",{onClick:m,style:{background:C.green,color:"#04261b"},className:"px-3 py-1.5 rounded-lg text-sm font-semibold"},"Salvar senha"),React.createElement("button",{onClick:()=>{i(!1),l("")},style:{background:C.card2,color:C.mut},className:"px-3 py-1.5 rounded-lg text-sm"},"Cancelar")))}const inp={background:C.bg,border:`1px solid ${C.line}`,color:C.txt,borderRadius:12,padding:"10px 12px",outline:"none",width:"100%"};function Field({label:o,children:e}){return React.createElement("label",{className:"block"},React.createElement("span",{className:"text-xs block mb-1",style:{color:C.mut}},o),e)}function AddSong({onClose:o,onSave:e,initial:n}){const t=!!n,[i,E]=useState(n?{t:n.t||"",n:n.n!=null?String(n.n):"",tom:n.tom||"",m:n.m||"Geral",u:n.u||"",yt:n.yt||"",cifra:n.cifra||""}:{t:"",n:"",tom:"",m:"Geral",u:"",yt:"",cifra:""}),[h,v]=useState(""),r=(m,A)=>E(u=>g(z({},u),{[m]:A})),f=async m=>{const A=m.target.files&&m.target.files[0];if(A){if(A.type&&A.type.indexOf("pdf")<0){v("Selecione um arquivo PDF.");return}v("Enviando\u2026");try{const u=(A.name||"musica.pdf").normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^A-Za-z0-9._-]+/g,"_"),p="up-"+Date.now().toString(36)+"-"+u,{error:D}=await SB.storage.from("uploads").upload(p,A,{upsert:!0,contentType:"application/pdf"});if(D){v("Falha no upload: "+D.message);return}const{data:c}=SB.storage.from("uploads").getPublicUrl(p);r("u",c.publicUrl),v("PDF enviado \u2713")}catch(u){v("Falha no upload.")}}},l=()=>{if(!i.t.trim())return;const m={t:i.t.trim(),tom:i.tom.trim(),m:i.m,u:i.u.trim(),yt:i.yt.trim(),cifra:i.cifra};i.n&&/^\d+$/.test(i.n)?m.n=Number(i.n):t&&!i.n&&(m.n=null),e(m)};return React.createElement("div",{style:{background:"rgba(0,0,0,0.6)"},className:"fixed inset-0 z-50 flex items-end sm:items-center justify-center"},React.createElement("div",{style:{background:C.card,border:`1px solid ${C.line}`},className:"w-full max-w-lg rounded-t-2xl sm:rounded-2xl max-h-[90vh] overflow-y-auto p-4 space-y-3"},React.createElement("div",{className:"flex items-center justify-between"},React.createElement("p",{className:"font-semibold",style:{color:C.gold}},t?"Editar m\xFAsica":"Nova m\xFAsica"),React.createElement("button",{onClick:o},React.createElement(X,{size:18,style:{color:C.mut}}))),React.createElement(Field,{label:"T\xEDtulo *"},React.createElement("input",{value:i.t,onChange:m=>r("t",m.target.value),style:inp,autoFocus:!0})),React.createElement("div",{className:"flex gap-2"},React.createElement(Field,{label:"N\xBA (opcional)"},React.createElement("input",{value:i.n,onChange:m=>r("n",m.target.value),style:inp,inputMode:"numeric"})),React.createElement(Field,{label:"Tom"},React.createElement("input",{value:i.tom,onChange:m=>r("tom",m.target.value),placeholder:"G / Sol",style:inp}))),React.createElement(Field,{label:"Momento da missa"},React.createElement("select",{value:i.m,onChange:m=>r("m",m.target.value),style:inp},MOMENTOS.map(m=>React.createElement("option",{key:m},m)))),React.createElement(Field,{label:"Link da cifra (PDF/Drive) \u2014 opcional"},React.createElement("input",{value:i.u,onChange:m=>r("u",m.target.value),placeholder:"https://\u2026",style:inp})),React.createElement("div",null,React.createElement("label",{style:{background:C.card2,color:C.txt,border:"1px dashed "+C.line},className:"flex items-center justify-center gap-2 py-2.5 rounded-xl cursor-pointer text-sm font-medium"},React.createElement(Upload,{size:16})," Enviar PDF da m\xFAsica",React.createElement("input",{type:"file",accept:"application/pdf",onChange:f,className:"hidden"})),h?React.createElement("p",{className:"text-xs mt-1",style:{color:h.indexOf("\u2713")>=0?C.green:h.indexOf("Enviando")>=0?C.mut:C.danger}},h):null),React.createElement(Field,{label:"Link do YouTube (bot\xE3o de v\xEDdeo) \u2014 opcional"},React.createElement("input",{value:i.yt,onChange:m=>r("yt",m.target.value),placeholder:"https://youtu.be/\u2026",style:inp})),React.createElement(Field,{label:"Cifra em texto (opcional \u2014 habilita transpor/rolar)"},React.createElement("textarea",{value:i.cifra,onChange:m=>r("cifra",m.target.value),rows:6,wrap:"off",style:g(z({},inp),{fontFamily:"ui-monospace, monospace",whiteSpace:"pre",overflowX:"auto"})})),React.createElement("button",{onClick:l,style:{background:C.green,color:"#04261b"},className:"w-full py-2.5 rounded-xl font-semibold flex items-center justify-center gap-1"},React.createElement(Check,{size:16})," ",t?"Salvar altera\xE7\xF5es":"Salvar m\xFAsica")))}function PdfViewer({url:o,title:e,onClose:n}){const t=driveEmbed(o);return React.createElement("div",{style:{background:"rgba(0,0,0,0.92)"},className:"fixed inset-0 z-[60] flex flex-col"},React.createElement("div",{style:{background:C.card,borderBottom:"1px solid "+C.line},className:"px-3 py-2 flex items-center gap-2"},React.createElement("button",{onClick:n,style:{background:C.card2,color:C.txt},className:"px-3 py-2 rounded-lg flex items-center gap-1 font-semibold shrink-0"},React.createElement(ChevronLeft,{size:18})," Voltar"),React.createElement("p",{className:"font-semibold truncate flex-1"},e),React.createElement("a",{href:o,target:"_blank",rel:"noreferrer",style:{color:C.blue},className:"text-xs flex items-center gap-1 shrink-0"},React.createElement(ExternalLink,{size:13})," nova aba")),React.createElement("iframe",{title:e,src:t,className:"flex-1 w-full",style:{border:0,background:"#fff"},allow:"autoplay"}))}function VideoViewer({url:o,title:e,onClose:n}){const t=String(o||"").match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/|live\/)|[?&]v=)([A-Za-z0-9_-]{6,})/),i=t?t[1]:"",E=i?"https://www.youtube.com/embed/"+i+"?rel=0&playsinline=1":"";return React.createElement("div",{style:{background:"rgba(0,0,0,0.92)"},className:"fixed inset-0 z-[60] flex flex-col"},React.createElement("div",{style:{background:C.card,borderBottom:"1px solid "+C.line},className:"px-3 py-2 flex items-center gap-2"},React.createElement("button",{onClick:n,style:{background:C.card2,color:C.txt},className:"px-3 py-2 rounded-lg flex items-center gap-1 font-semibold shrink-0"},React.createElement(ChevronLeft,{size:18})," Voltar"),React.createElement("p",{className:"font-semibold truncate flex-1"},e),React.createElement("a",{href:o,target:"_blank",rel:"noreferrer",style:{color:C.blue},className:"text-xs flex items-center gap-1 shrink-0"},React.createElement(ExternalLink,{size:13})," YouTube")),React.createElement("div",{className:"flex-1 flex items-center justify-center p-2"},E?React.createElement("div",{style:{position:"relative",width:"100%",maxWidth:900,aspectRatio:"16 / 9"}},React.createElement("iframe",{title:e,src:E,style:{border:0,width:"100%",height:"100%",borderRadius:12},allow:"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture",allowFullScreen:!0})):React.createElement("p",{style:{color:C.mut}},"Link de v\xEDdeo inv\xE1lido. ",React.createElement("a",{href:o,target:"_blank",rel:"noreferrer",style:{color:C.blue},className:"underline"},"Abrir link"))))}function LockModal({hasPin:o,onClose:e,onSubmit:n}){const[t,i]=useState(""),[E,h]=useState(""),v=()=>{const r=n(t);r?h(r):e()};return React.createElement("div",{style:{background:"rgba(0,0,0,0.6)"},className:"fixed inset-0 z-[70] flex items-end sm:items-center justify-center"},React.createElement("div",{style:{background:C.card,border:"1px solid "+C.line},className:"w-full max-w-sm rounded-t-2xl sm:rounded-2xl p-4 space-y-3"},React.createElement("div",{className:"flex items-center justify-between"},React.createElement("p",{className:"font-semibold",style:{color:C.gold}},o?"Modo edi\xE7\xE3o":"Criar senha de edi\xE7\xE3o"),React.createElement("button",{onClick:e},React.createElement(X,{size:18,style:{color:C.mut}}))),React.createElement("p",{className:"text-xs",style:{color:C.mut}},o?"Digite a senha para editar m\xFAsicas e equipes neste aparelho.":"Defina uma senha (m\xEDn. 4 d\xEDgitos). Quem tiver a senha poder\xE1 editar o repert\xF3rio e as equipes."),React.createElement("input",{value:t,onChange:r=>{i(r.target.value),h("")},onKeyDown:r=>r.key==="Enter"&&v(),type:"password",inputMode:"numeric",placeholder:o?"Senha":"Nova senha",style:inp,autoFocus:!0}),E?React.createElement("p",{className:"text-xs",style:{color:C.danger}},E):null,React.createElement("button",{onClick:v,style:{background:C.gold,color:"#2b2100"},className:"w-full py-2.5 rounded-xl font-semibold flex items-center justify-center gap-1"},React.createElement(Unlock,{size:16})," ",o?"Entrar":"Criar e entrar")))}const _root=ReactDOM.createRoot(document.getElementById("root"));_root.render(React.createElement(App,null));try{window.__booted=!0}catch(o){}
